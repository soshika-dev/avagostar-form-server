...
- Dashboard transaction amounts are normalized to AED (UAE Dirham) in list/summary responses (with original currency fields preserved on list/detail)
- Currency conversion uses the free ExchangeRate-API endpoint (`open.er-api.com`) with fallback defaults
- USD conversion rates can still be overridden via `USD_EXCHANGE_RATES` env var to pin/adjust specific rates
...
| `USD_EXCHANGE_RATES` | JSON object of currency->USD rates (example: `{"EUR":1.17,"AED":0.2723}`); overrides fetched API rates | _unset_ (uses free API rates, then built-in defaults if API is unavailable) |
